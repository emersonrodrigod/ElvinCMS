      ...
      <div class="box box-primary">
        <div class="box-body">  
          @if ($gallery->items->count())
            @foreach ($gallery->items as $item)
                <div class="col-md-3 box-photo row-{{$item->id}}">
                  <div class="destroy" data-id="{{$item->id}}" data-table="/admin/gallery-items"><span class="btn btn-danger"><i class="fa fa-trash"></i></span></div>
          ...
          @else
            <!--
            <div class="col-md-12">
              @lang('cms_galleries.There isn\'t any image or video in') {{$gallery->title}}
            </div>
            -->
            <div class="col-md-3 box-photo" onClick="$('html, body').animate({scrollTop: $('#add-item').offset().top}, 1000);">
              <div class="caption"><span class="label label-default"></span></div>
      ...
      <div class="box box-primary">
        <div class="box-body">  
          ...
          <div class="box-footer">
            <input type="submit" value="{{@trans('Add to gallery')}}" class="btn btn-primary" id="add-to-gallery" data-gallery-id="{{$gallery->id}}">
          </div>
        </div>
      </div>
    </section>
...
<div>
      <div class="col-md-3 box-photo" v-for="item in items">
          <div class="destroy"><span class="btn btn-danger"><i class="fa fa-trash"></i></span></div>
          <div class="caption" v-for="d in item.details">
            <span class="label label-default">
              <img class="flag flag-xs" src="/cms/img/flags/en.png"> {{d.caption}}
          </span>
          </div>
           <!-- @if(starts_with($item->image, 'http'))
              <img class="img-responsive" src="{{$item->image}}">
